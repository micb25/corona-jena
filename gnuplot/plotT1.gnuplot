...
set label 2 at graph 0.99, 0.93 "{/*0.75 Quelle: TMASGFF und RKI}" right textcolor ls 0
...
plot  \
  ...
  "<awk -F, '{if ($2==\"TH\") print $1,$4}' ../data/rki_th_by_date/cases_by_day_and_region.csv | awk '{if ($2 >= 0) print $0}' | awk 'BEGIN{ov=0}{dv=$2-ov;ov=$2;print $1,$2,dv}' | tail -n 1" using 1:2:(sprintf("%i\n(%+i)", $2, $3)) with labels point pt 7 right offset char  6.5, -0.4 tc ls 4 notitle, \
  ...
  "<awk -F, '{if ($2==\"TH\") print $1,$4}' ../data/rki_th_by_date/cases_by_day_and_region.csv" using 1:(filter_neg($2)) with linespoints ls 4 title "Genesene", \
