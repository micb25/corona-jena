...
set output '../plot2_GRZ_Trend1604.png'

# stats for x
stats "<awk -F, '{if (NR>1) print $1, $2}' ../data/cases_grz.csv" using 1 nooutput
...
bo = 0.15
fo(x) = ao * exp( bo * x )
fit fo(x) "<awk -F, '{if(NR>1) print $1, $2}' ../data/cases_grz.csv" using ((filterx($1) - xmin_f)/86400):(filter_neg($2)) via ao, bo
