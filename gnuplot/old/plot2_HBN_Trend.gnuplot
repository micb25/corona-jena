...
set output '../plot2_HBN_Trend.png'

# stats for x
stats "<awk -F, '{if (NR>1) print $1, $2}' ../data/cases_hbn.csv" using 1 nooutput
...
bo = 0.15
fo(x) = ao * exp( bo * x )
fit fo(x) "<awk -F, '{if(NR>1) print $1, $2}' ../data/cases_hbn.csv" using ((filterx($1) - xmin_f)/86400):(filter_neg($2)) via ao, bo
...
plot  \
  ...
  "<awk -F, '{if (NR>1) print $1, $2}' ../data/cases_hbn.csv" using 1:(filter_neg($2)) with linespoints ls 1 title "bestätigte Fälle", \
