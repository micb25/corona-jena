...
d = 1.0
f(x) = a / ( 1 + exp(  - b * ( x - c ) ) )
fit f(x) "<awk '!_[$2]++' ../data/cases_thuringia_rki.dat | awk '{if ($1 <= 1585951200) print $0}'" using (($1 - xmin_o) / 86400):(filter_neg($2)) via a, b, c
...
plot  \
  ...
  [xmin_o:xmax_o] '+' using 1:(fmin(($1 - xmin_o)/86400)):(fmax((x - xmin_o)/86400)) with filledcurves closed ls 2 title "{/*0.75 stat. Fehlerbereich sigmoidaler Trend (04.04.)}", \
  [xmin_o:xmax_o] f((x - xmin_o)/86400) w l ls 12 title "sigmoidaler Trend (04.04.)", \
