...
plot  \
  ...
  "<awk -F, 'BEGIN{for(i=0;i<7;i++){a[i]=0}} { if (NR > 1) {for(i=0; i<6; i++){a[i]=a[i+1]};a[6]=$4;b=0;for(i=0;i<7; i++){b=b+a[i]};print $1,b/7}}' ../data/rki_th/reproduction_rates_th.csv" using 1:(filter_neg($2)) with lines ls 12  lc rgb "#800000" lw 3.0 title "7-Tage-Mittelwert von {/Linux-Libertine-O-Italic R}"
