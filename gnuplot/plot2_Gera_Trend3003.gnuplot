...
bo = 0.15
fo(x) = ao * exp( bo * x )
fit fo(x) "<awk '!_[$2]++' ../data/cases_gera.dat" using ((filterx($1) - xmin_f)/86400):(filter_neg($2)) via ao, bo
...
plot  \
  ...
  [ xmin_f < xmin_o ? xmin_o : xmin_f :xmax_f] fo((x - xmin_f)/86400) w l ls 2 title "exponentieller Trend (letzte 7 Tage)", \
