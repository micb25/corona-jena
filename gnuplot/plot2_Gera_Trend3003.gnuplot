...
# stats for x
stats "<awk '!_[$2]++' ../data/cases_gera.dat" using 1 nooutput
xmin = int(STATS_min) - 1 * 86400
xmin_o = int(STATS_min)
xmin_f = 1585584004 # 30.03.2020
...
bo = 0.15
fo(x) = ao * exp( bo * x )
fit fo(x) "<awk '!_[$2]++' ../data/cases_gera.dat | awk '{if ($1 >= 1585584004) print $0 }'" using (($1 - xmin_f)/86400):(filter_neg($2)) via ao, bo
...
plot  \
  ...
  [xmin_f:xmax_f] '+' using 1:(fomin(($1 - xmin_f)/86400)):(fomax((x - xmin_f)/86400)) with filledcurves closed ls 2 title "{/*0.75 stat. Fehlerbereich Trend (ab 30.03.)}", \
  [xmin_f:xmax_f] fo((x - xmin_f)/86400) w l ls 2 title "exponentieller Trend (ab 30.03.)", \
  "<awk '!_[$2]++' ../data/cases_gera.dat" using 1:2 with linespoints ls 1 title "bestätigte Fälle"
