...
# stats for x
stats "<awk -F, '{print $1, $2}' ../data/cases_gera.csv" using 1 nooutput
...
bo = 0.15
fo(x) = ao * exp( bo * x )
fit fo(x) "<awk -F, '{print $1, $2}' ../data/cases_gera.csv" using ((filterx($1) - xmin_f)/86400):(filter_neg($2)) via ao, bo
...
plot  \
  ...
  "<awk -F, '{print $1, $2}' ../data/cases_gera.csv " using 1:(filter_neg($2)) with linespoints ls 1 title "bestätigte Fälle", \
  [ xmin_f < xmin_o ? xmin_o : xmin_f :xmax_f] fo((x - xmin_f)/86400) w l ls 12 notitle
