...
# latest update
date_cmd = sprintf("%s", "`awk -F, '{print "@"$1}' ../data/cases_weimar.csv | tail -n 1 | xargs date +"%d.%m.%Y, %H:%M" -d`")
...
set label 2 at graph 0.99, 0.93 "{/*0.75 Quelle: Stadt Weimar}" right textcolor ls 0
