...
# get maximum value
stats "<awk -F, 'BEGIN{d=0;w=-51;tmin=1584316800}{if((NR>1)&&($1>=1584316800)){if ($1<=tmin+7*86400){w+=$2-d;d=$2;}else{print tmin,w;tmin=tmin+7*86400;w=0;}}}END{if (w>0) print tmin, w}' ../data/cases_th_sums.csv" using 2 name "A" nooutput
...
set label 1 at graph 0.98, 0.95 "{/Linux-Libertine-O-Bold bestätigte Neuinfektionen pro Kalenderwoche in Thüringen}" right textcolor ls 0
set label 2 at graph 0.98, 0.90 update_str right textcolor ls 0

# data
plot  \
  "<awk -F, 'BEGIN{d=0;w=-51;tmin=1584316800}{if((NR>1)&&($1>=1584316800)){if ($1<=tmin+7*86400){w+=$2-d;d=$2;}else{print tmin,w;tmin=tmin+7*86400;w=0;}}}END{if (w>0) print tmin, w}' ../data/cases_th_sums.csv" using 1:2 with boxes lt rgb "#0000FF" notitle, \
  "<awk -F, 'BEGIN{d=0;w=-51;tmin=1584316800}{if((NR>1)&&($1>=1584316800)){if ($1<=tmin+7*86400){w+=$2-d;d=$2;}else{print tmin,w;tmin=tmin+7*86400;w=0;}}}END{if (w>0) print tmin, w}' ../data/cases_th_sums.csv" using 1:2:2 with labels point pt 7 ps 0 center offset char -0.5, 0.5 tc ls 1 notitle, \
