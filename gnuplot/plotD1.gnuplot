...
plot  \
  ...
  \
  "<awk -F, '{if ( NR>1) {a=$1;c=b;b=$6}}END{print a, b, b-c}' ../data/divi_db_th/divi_data_th.csv | tail -n 1" using 1:2:(sprintf("%i (%+i)", $2, $3)) with labels point ls 17 ps 0.0 left offset char  0.3, +0.05 tc ls 17 notitle, \
  "<awk -F, '{if ( NR>1) {a=$1;c=b;b=$5}}END{print a, b, b-c}' ../data/divi_db_th/divi_data_th.csv | tail -n 1" using 1:2:(sprintf("%i (%+i)", $2, $3)) with labels point ls 18 ps 0.0 left offset char  0.3, +0.05 tc ls 18 notitle
