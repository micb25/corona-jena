...
plot \
     "<awk -F, '{a[$1]+=$4;b[$1]+=$8;c[$1]+=$7}END{for(i in a) print int(i/86400)*86400,a[i],b[i],c[i]}' ../data/cases_thuringia.dat | sort -n -k1 | tail -n 1" u (xpos):(ypos(1)):(sprintf("%i bestätigte Fälle in Thüringen", $2)) w labels left offset 2.5, 0, \
     ...
     "<awk -F, '{a[$1]+=$4;b[$1]+=$8;c[$1]+=$7}END{for(i in a) print int(i/86400)*86400,a[i],b[i],c[i]}' ../data/cases_thuringia.dat | sort -n -k1 | tail -n 1" u (xpos):(ypos(2)):(sprintf("%i aktive Fälle* (%.1f%%)", $2 - B_max - $4, 100*($2-B_max-$4)/A_sum)) w labels left offset 2.5, 0, \
     ...
     "<awk -F, '{a[$1]+=$4;b[$1]+=$8;c[$1]+=$7}END{for(i in a) print int(i/86400)*86400,a[i],b[i],c[i]}' ../data/cases_thuringia.dat | sort -n -k1 | tail -n 1" u (xpos):(ypos(3)):(sprintf("%i Genesene* (%.1f%%)", B_max, 100*B_max/A_sum)) w labels left offset 2.5, 0, \
     ...
     "<awk -F, '{a[$1]+=$4;b[$1]+=$8;c[$1]+=$7}END{for(i in a) print int(i/86400)*86400,a[i],b[i],c[i]}' ../data/cases_thuringia.dat | sort -n -k1 | tail -n 1" u (xpos):(ypos(4)):(sprintf("%i Verstorbene (%.1f%%)", $4, 100*$4/A_sum)) w labels left offset 2.5, 0, \
