...
plot  \
  ...
  "<awk -F, '{if ( NR>1) {a=$1;c=b;b=$3-$5}}END{print a, b, b-c}' ../data/divi_db_th/divi_data_th.csv | tail -n 1" using 1:(filter_neg($2)):(sprintf("%i\\n(%+i)", $2, $3)) with labels point ls 18 ps 0.0 right offset char  5.0, +0.4 tc ls 19 notitle, \
  "<awk -F, '{if ( NR>1) {a=$1;c=b;b=$2}}END{print a, b, b-c}' ../data/divi_db_th/divi_data_th.csv | tail -n 1" using 1:(filter_neg($2)):(sprintf("%i\\n(%+i)", $2, $3)) with labels point ls 18 ps 0.0 right offset char  5.0, +0.4 tc ls 16 notitle
