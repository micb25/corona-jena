...
# stats for x
stats "<awk -F, '{a[$1]+=$4}END{for(i in a) print int(i/86400)*86400,a[i]}' cases_thuringia.dat | sort -n -k1" using 1 nooutput
set xrange [ STATS_min - 1.5 * 86400 : STATS_max + 1.5 * 86400 ]

# stats for y
stats "<awk -F, '{a[$1]+=$4}END{for(i in a) print int(i/86400)*86400,a[i]}' cases_thuringia.dat | sort -n -k1" using 2 nooutput
...
plot  \
  ...
  "<awk -F, '{if ( $1 < 1585180800 ) a[$1]+=$8}END{for(i in a) print int(i/86400)*86400,a[i]}' cases_thuringia.dat | sort -n -k1" using 1:2 with linespoints ls 4 title "Genesene (bis 25.03.)", \
